...
one_worldDBName = input("input worldDB in SQL's name(like: one_world or mangos1)  : ")

if len(path) < 2 or len(selectSqlType) == 0 or len(sqlHost) == 0 or len(sqlPort) == 0 or len(sqlUserName) == 0 or len(one_worldDBName) == 0:
