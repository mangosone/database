...
BEGIN
    ...
    IF (@cCurResult = @cOldResult) THEN    -- Does the current version match the expected version
		...
		SET @gossip_entry := (SELECT MAX(`entry`) FROM `gossip_menu`);
		...
		DELETE FROM `gossip_menu` WHERE  `text_id` = 7045;
		INSERT INTO `gossip_menu` (`entry`, `text_id`, `script_id`, `condition_id`) VALUES (@gossip_entry+1,7045,0,0);

		UPDATE `creature_template` SET `GossipMenuId` = @gossip_entry+1 WHERE `Entry` = 14531;
		...
		DELETE FROM `creature_movement` WHERE `id` = 43224;
