...
BEGIN
    ...
    IF (@cCurResult = @cOldResult) THEN    -- Does the current version match the expected version
		...
		DELETE FROM `gossip_menu` WHERE  `text_id` = 7044;
		INSERT INTO `gossip_menu` (`entry`, `text_id`, `script_id`, `condition_id`) VALUES (@gossip_entry+1,7044,0,0);

		UPDATE `creature_template` SET `GossipMenuId` = @gossip_entry+1 WHERE `Entry` = 14536;
		...
		DELETE FROM `creature` WHERE `guid` = 43224;
